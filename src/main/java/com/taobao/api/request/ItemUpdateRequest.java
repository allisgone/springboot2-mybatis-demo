...
public class ItemUpdateRequest extends BaseTaobaoRequest<ItemUpdateResponse> implements TaobaoUploadRequest<ItemUpdateResponse> {
	...
	* 食品添加剂
	...
	/** 
	* 健字号，保健品膳食营养补充剂 这个类目下特有的信息，此类目下无需填写生产许可证编号（QS），如果填写了生产许可证编号（QS）将被忽略不保存；保存宝贝时，标题前会自动加上健字号产品名称一起作为宝贝标题；
	 */
	...
	* 运费承担方式。运费承担方式。可选值:seller（卖家承担）,buyer(买家承担);
	...
	/** 
	* 全球购商品采购地（地区国家）,默认值只在全球购商品采购地（库存类型选择情况生效），地区国家值为（美国, 香港, 日本, 英国, 新西兰, 德国, 韩国, 荷兰, 澳洲, 法国, 意大利, 台湾, 澳门, 加拿大, 瑞士, 西班牙, 泰国, 新加坡, 马来西亚, 菲律宾, 其他）
	 */
	...
	* 全球购商品卖家包税承诺，当值为true时，代表卖家承诺包税。
	...
	/** 
	* 全球购商品采购地（库存类型）全球购商品有两种库存类型：现货和代购 参数值为1时代表现货，值为2时代表代购。注意：使用时请与 全球购商品采购地（地区国家）配合使用
	 */
	...
	* 淘宝租赁扩展信息
	 */
	private String leaseExtendsInfo;
	...
	* 无线的宝贝描述
	...
	public void setLeaseExtendsInfo(String leaseExtendsInfo) {
		this.leaseExtendsInfo = leaseExtendsInfo;
	}

	public String getLeaseExtendsInfo() {
	...
	public Map<String, String> getTextParams() {
		...
		txtParams.put("lease_extends_info", this.leaseExtendsInfo);
